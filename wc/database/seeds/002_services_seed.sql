-- Idempotent seeds for Services module (permissions, admin page/menu, role grants)
SET NAMES utf8mb4;
SET time_zone = '+00:00';
...
FROM (
    SELECT 'services' AS module, 'view' AS action, 'View services' AS description UNION ALL
    SELECT 'services', 'create', 'Create services' UNION ALL
    SELECT 'services', 'edit', 'Edit services' UNION ALL
    SELECT 'services', 'delete', 'Soft-delete services' UNION ALL
    SELECT 'services', 'restore', 'Restore services'
) AS m
WHERE NOT EXISTS (
...
-- Admin: all services permissions
INSERT INTO role_permissions (role_id, permission_id, created_at)
SELECT r.id, p.id, UTC_TIMESTAMP()
FROM roles r
CROSS JOIN permissions p
...
WHERE r.code_active = 'editor'
  AND p.deleted_at IS NULL
  AND p.code IN ('services.view', 'services.create', 'services.edit')
  AND NOT EXISTS (
      SELECT 1 FROM role_permissions rp
      WHERE rp.role_id = r.id AND rp.permission_id = p.id AND rp.deleted_at IS NULL
  );
...
SELECT 'admin', 'Services', 'services', 'services', 1, UTC_TIMESTAMP()
WHERE NOT EXISTS (
    SELECT 1 FROM pages p WHERE p.type_slug_active = 'admin:services'
);
...
FROM menus m
JOIN pages p ON p.page_type = 'admin' AND p.slug = 'services' AND p.deleted_at IS NULL
