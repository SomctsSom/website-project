...
SELECT 'admin', parent.id, 'Profile Overviews', NULL, '/profile-overviews', 45, 1, UTC_TIMESTAMP()
FROM menus parent
WHERE parent.menu_type = 'admin'
  ...
  AND NOT EXISTS (
      ...
      WHERE m.menu_type = 'admin' AND m.parent_menu_id = parent.id AND m.title = 'Profile Overviews' AND m.deleted_at IS NULL
  );
