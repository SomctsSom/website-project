...
SELECT 'admin', parent.id, 'Features', NULL, '/features', 47, 1, UTC_TIMESTAMP()
FROM menus parent
WHERE parent.menu_type = 'admin'
  ...
  AND NOT EXISTS (
      ...
      WHERE m.menu_type = 'admin' AND m.parent_menu_id = parent.id AND m.title = 'Features' AND m.deleted_at IS NULL
  );
