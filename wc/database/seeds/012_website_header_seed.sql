-- Website Header permissions, admin page, menu
SET NAMES utf8mb4;
SET time_zone = '+00:00';
...
FROM (
    SELECT 'website_header' AS module, 'view' AS action, 'View website header settings' AS description UNION ALL
    SELECT 'website_header', 'edit', 'Edit website header settings'
) AS m
WHERE NOT EXISTS (
...
WHERE r.code_active IN ('super_admin', 'admin')
  AND p.code LIKE 'website_header.%'
...
WHERE r.code_active = 'editor'
  AND p.deleted_at IS NULL
  AND p.code IN ('website_header.view', 'website_header.edit')
  AND NOT EXISTS (
      SELECT 1 FROM role_permissions rp
      WHERE rp.role_id = r.id AND rp.permission_id = p.id AND rp.deleted_at IS NULL
  );
...
SELECT 'admin', 'Website Header', 'website-header', 'website_header', 1, UTC_TIMESTAMP()
WHERE NOT EXISTS (
    SELECT 1 FROM pages p WHERE p.type_slug_active = 'admin:website-header'
);

INSERT INTO menus (menu_type, parent_menu_id, title, icon, url, sort_order, is_active, created_at)
SELECT 'admin', parent.id, 'Website Header', NULL, '/website-header', 65, 1, UTC_TIMESTAMP()
FROM menus parent
WHERE parent.menu_type = 'admin'
  ...
  AND NOT EXISTS (
      ...
      WHERE m.menu_type = 'admin' AND m.title = 'Website Header' AND m.url = '/website-header' AND m.deleted_at IS NULL
  );

INSERT INTO menu_pages (menu_id, page_id, sort_order, created_at)
SELECT m.id, p.id, 0, UTC_TIMESTAMP()
FROM menus m
JOIN pages p ON p.page_type = 'admin' AND p.slug = 'website-header' AND p.deleted_at IS NULL
WHERE m.menu_type = 'admin' AND m.title = 'Website Header' AND m.url = '/website-header' AND m.deleted_at IS NULL
  AND NOT EXISTS (
